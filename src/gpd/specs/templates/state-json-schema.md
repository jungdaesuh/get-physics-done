...
| `session_goal` | `SessionGoal \| null` | `null` | User-stated session goal (RES-932) pinned into every agent init bundle | **Authoritative** (JSON-only, from `gpd goal set`) |
...
For JSON-only fields (`project_contract`, `resolved_questions`, `approximations`, `convention_lock`, `propagated_uncertainties`, `continuation`, `contract_alignment`, `session_goal`, and structured `intermediate_results`): state.json is sole authority. STATE.md renders a lossy view where projections exist.
