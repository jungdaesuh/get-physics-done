...
| `GPD/STATE.md` | Editable human-readable state document; reconstruction input when the recovery ladder falls back to text | Reconstruction surface |
| `GPD/phases/.../.continue-here.md` | Temporary continuation handoff artifact written by `gpd:pause-work`; only meaningful through the canonical continuation hierarchy | Non-authoritative continuity artifact |
| Execution lineage | Append-only execution provenance | Authoritative for history only |
| Derived execution head / `GPD/observability/current-execution.json` | Latest live execution snapshot | Advisory status surface |

The shared resolver is `gpd --raw resume`. Canonical state in `state.json.continuation` wins first; bounded segments and recorded handoff fields define resume targets; derived execution heads and `.continue-here.md` provide current context but do not define separate authority.
...
- Append-only execution lineage and its derived head
...
- **Runtime configuration**: Installed prompts and editor-specific hooks are written to runtime-local directories (e.g., project-local config directories managed by the active coding assistant). These are regenerated by `gpd install`. GPD has no built-in MCP server registrations to regenerate; machine-facing GPD functionality is reachable through the `gpd` CLI instead. If the optional Wolfram integration is enabled, its `gpd-wolfram` MCP registration is the one exception -- it is machine-local and regenerated the same way.
- **Legacy MCP server entries**: if an older install left `gpd-*` MCP server registrations in the runtime config, `gpd install` scrubs them on upgrade; they are not part of the portable project state.
- **Observability streams** (`GPD/observability/`): Session telemetry remains primarily machine-local. Execution lineage and the derived head are portable project history, while `GPD/observability/current-execution.json` is advisory live status. It must not set `active_resume_kind="bounded_segment"` unless the canonical resume view names a valid bounded segment.
...
3. **Run the installer**: `{GPD_BOOTSTRAP_COMMAND}` (or the equivalent install pathway). This creates a fresh managed venv, installs the GPD package, writes runtime-local configuration, scrubs any legacy `gpd-*` MCP server entries, and registers the optional `gpd-wolfram` MCP integration if it was previously enabled.
...
- **Task level**: bounded execution state, active resume pointer, interrupted agents, and live review gates
...
2. **Resume artifacts**: Canonical `state.json.continuation.bounded_segment` is the durable bounded-segment authority. Recorded handoff files become `continuity_handoff_file` / `active_resume_pointer` only when they are part of the canonical resume view; missing recorded handoffs stay advisory.
...
The resume workflow ranks three recovery families into `resume_candidates`: bounded execution snapshot, recorded handoff, and interrupted agent. `gpd --raw resume` emits the canonical top-level list. A `.continue-here.md` file alone is a temporary handoff artifact, not a resume authority.
...
The shared resume resolver turns the storage authority chain into one canonical continuation view:
...
4. Detects derived execution heads, recorded handoff pointers, interrupted agents, pending reviews, and non-blocking machine-change advisories.
